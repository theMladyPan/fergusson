...
async def get_history(session: AsyncSession, chat_id: str, limit: int = 20) -> List[ModelMessage]:
    ...
    result = await session.execute(
        select(Summary).where(Summary.chat_id == chat_id).order_by(Summary.timestamp.desc()).limit(1)
    )
...
async def check_and_compact(session: AsyncSession, chat_id: str, archiver_agent: "Archiver") -> None:
    ...
    stmt = select(func.count(Message.id)).where(Message.chat_id == chat_id, Message.is_valid == True)  # noqa: E712
