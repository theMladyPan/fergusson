...
*   **Model Configuration:** The agent now loads `SMART_MODEL`, `FAST_MODEL`, and `ROUTER_MODEL` directly from environment variables as native PydanticAI `provider:model` strings. Fergusson keeps a thin wrapper only for OpenAI and Google direct-provider strings so existing retry and Logfire instrumentation behavior is preserved.
*   **Auto-Routing First Stage:** Before invoking the full Core Agent, `AgentManager.run` consults a cheap `ROUTER_MODEL` (`src/agent/router.py`, default `google-gla:gemini-3.5-flash-lite`). The router returns a structured decision `{action, reply}`: `answer` returns the router's reply directly (no smart-model cost; only read-only `read_file_content` + Exa `web_search` + the four read-only Google Workspace tools are available to it), `clarify` returns one concise disambiguation question directly (also no smart-model cost; used for vague requests to save the expensive escalation round trip), `escalate` runs the full Core Agent on `SMART_MODEL` (default `google-gla:gemini-3.6-flash`) with all tools; on escalate the router's gathered tool results are extracted (`_extract_tool_returns`) and injected into `AgentDeps.router_context`, surfaced to the Core Agent via a dynamic system prompt so it continues from the router's reads instead of redoing them. The router is fail-fast: tool errors, timeouts, invalid structured output, or usage-limit exhaustion all collapse to `escalate` (escalating is always safe). It sees only the most recent `Settings.router.history_window` history messages to stay cheap, while the Core Agent still receives full history. Routing is gated by `Settings.router.enabled` (default on); set `ROUTER_ENABLED=false` to restore direct-Core-Agent behavior.
*   **Native Google Workspace Tools:** `src/tools/gws.py` wraps the `gws` CLI so the agent (and the cheap router) read Gmail/Calendar/Drive directly instead of shelling out through bash or the `gws` skill. `list_inbox_emails`, `get_contact`, `list_upcoming_events`, and `search_drive_docs` are read-only and registered on both the router and the Core Agent; `create_calendar_event` (with optional attendees and a Google Meet link) is a write and stays on the Core Agent. The `gws` subprocess runs with a sanitized environment (`GOOGLE_APPLICATION_CREDENTIALS` is stripped so the STT service-account ADC can never leak in), and auth failures fail fast via `ModelRetry` pointing to the `gws-debug` skill. Email/event/doc summaries use `Settings.summary_model` (default `openrouter:openai/gpt-oss-20b:nitro` with `SUMMARY_REASONING_EFFORT=low`) and run in parallel via `asyncio.gather`; if the summary model is unavailable, tools degrade to raw data.
...
*   **Voice Services:** The run loop (`src/runners.py`) handles voice-for-voice turns via two services in `src/services/`, both degrading to a no-op when unconfigured or on error so the agent keeps working without voice. **STT** (`src/services/chirp3.py`) transcribes audio media attachments using Google Chirp 3 (`google-cloud-speech` v2, online inline `recognize`, `model="chirp_3"`, `language_codes=["auto"]`), configured via `STT_*` env vars (`Settings.stt`), including `STT_CREDENTIALS_FILE`; Chirp loads that service-account key explicitly so the process and generic shell children do not expose it as ADC to tools such as `gws`. **TTS** (`src/services/cartesia.py`) synthesizes replies via Cartesia (`cartesia` SDK, `AsyncCartesia`, mp3 output, model `sonic-3.5`), configured via `CARTESIA_*` env vars (`Settings.cartesia`). The router always escalates speech tasks to the Core Agent, which receives native wrappers `transcribe_audio` and `synthesize_speech` from `src/tools/audio.py`, removing any need for bash/ffmpeg/Whisper/ad-hoc Python; synthesized files can be attached through `send_message_to_channel(media_paths=[...])`. Both services replace the former single-provider ElevenLabs path.
...
*   **Proactive Messaging:** To allow the agent to send messages across boundaries (e.g., asking it in the CLI to ping you on Discord), the Core Agent is equipped with two specific tools:
    ...
    2.  `send_message_to_channel(channel, message, chat_id, media_paths=None)`: Injects a message and optional attachments directly into the Redis outbound queue for the target channel.
...
Neo4j adds an optional structured long-term memory layer on top of the shared SQLite thread. `MEMORY.md` remains only as a sparse anchor sheet for critical identifiers.
