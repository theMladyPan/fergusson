...
*   **Auto-Routing First Stage:** Before invoking the full Core Agent, `AgentManager.run` consults a cheap `ROUTER_MODEL` (`src/agent/router.py`, default `google-gla:gemini-3.5-flash-lite`). The router returns a structured decision `{action, reply}`: `answer` returns the router's reply directly (no smart-model cost; only read-only `read_file_content` + Exa `web_search` are available to it), `clarify` returns one concise disambiguation question directly (also no smart-model cost; used for vague requests to save the expensive escalation round trip), `escalate` runs the full Core Agent on `SMART_MODEL` (default `google-gla:gemini-3.6-flash`) with all tools. The router is fail-fast: tool errors, timeouts, invalid structured output, or usage-limit exhaustion all collapse to `escalate` (escalating is always safe). It sees only the most recent `Settings.router.history_window` history messages to stay cheap, while the Core Agent still receives full history. Routing is gated by `Settings.router.enabled` (default on); set `ROUTER_ENABLED=false` to restore direct-Core-Agent behavior.
